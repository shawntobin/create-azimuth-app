...
// NOTE: folktale should be replaced with fp-ts
import { Just, Nothing } from "folktale/maybe";
import { Atom, dwim, jam as serialJam } from "@urbit/nockjs";
...
const jam = (seed) => {
  const hex = serialJam(seed).toString().slice(2);
...
export const compileNetworkKey = (pair, point, revision) => {
  ...
  const sed = dwim(
    Atom.fromInt(point),
    Atom.fromInt(revision),
    Atom.fromString(bnsec.toString()),
    Atom.fromInt(0)
  );
...
export const compileMultiKey = (point, keys) => {
  ...
  const fed = dwim(
    dwim(Atom.fromInt(1), Atom.fromInt(0)), // version
...
export const deriveNetworkSeedFromAuthToken = (point, authToken, revision) => {
  ...
  const networkSeed = shas(authToken, salt).toString("hex").slice(0, 32);
